...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        Phaser[] p = new Phaser[tasks+2];
        for(int m=0,K=p.length;m<K;m++){
            p[m] = new Phaser(1);
        }
        Thread[] threads = new Thread[tasks];
        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    int returnVal = p[i].arrive();
                    if(i>=1){p[i-1].awaitAdvance(returnVal);}
                    if(i<tasks-1){p[i+1].awaitAdvance(returnVal);}
                    /*if(index>1) p[index-1].awaitAdvance(returnVal);
                    if(index<tasks) p[index+1].awaitAdvance(returnVal);*/
